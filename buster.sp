...
public Plugin:myinfo = 
{
	...
	author = "Lododar",
	description = "Modified verison of MasterOfTheXP's Be the Robot: Sentry Buster module developed for Team Fortress 2 Classified",
	version = PLUGIN_VERSION,
	url = "https://forums.alliedmods.net/showthread.php?t=193067"
}
...
public OnPluginStart()
{
	...
	cvarBusterAnnounce = CreateConVar("sm_betherobot_buster_announce","3","Who should the Administrator warn about a Sentry Buster's presence? 1=Enemy team, 2=Sentry Buster's team, 3=Both, 0=No one. Default is 3 (both teams)", FCVAR_NONE, true, 0.0);
...
stock bool:ToggleBuster(client, bool:toggle = bool:2)
{
	...
	if (toggle == true || (toggle == bool:2 && Status[client] == BusterStatus_Human))
	{
		...
		TF2_RemoveWeaponSlot(client, 4); // remove destruction-pda
		TF2_RemoveWeaponSlot(client, 5); // remove any misc weapon (???)
		new String:atts[128];
	...
	else if (!toggle || (toggle == bool:2 && Status[client] == BusterStatus_Buster))
	{
		...
		AcceptEntityInput(client, "SetCustomModel");
		Status[client] = BusterStatus_Human;
        ForcePlayerSuicide(client, false); // Forcing suicide as TF2_RegeneratePlayer is not supported on TF2C
